...
def get_sorted_wp_filtered_to_worker(worker, models_list=None, blacklist=None, priority_user_ids=None, page=0):
    ...
    if "z-image-turbo" in models_list_lower:
        all_active_wps = (
            ...
            .join(WPModels)
            .filter(
                ImageWaitingPrompt.active.is_(True),
                ImageWaitingPrompt.faulted.is_(False),
        ...
        for wp in all_active_wps:
            ...
            logger.warning(
                f"[DB_MATCH]     Worker: nsfw={worker.nsfw}, unsafe_ip={worker.allow_unsafe_ipaddr}, "
                f"speed={worker.speed}, r2_capable={r2_capable}",
            )
