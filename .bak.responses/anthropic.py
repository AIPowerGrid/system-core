...
async def create_message(
    ...
    # Legacy horde bookkeeping rows only exist for legacy (Haidra) keys — v2
    # account ids don't fit the integer FK, and nothing v2 reads these.
    # (Until the #53 Anthropic passthrough lands, the worker receives the
    # flattened `prompt` and serves it via its legacy single-message fallback.)
...
async def _stream_anthropic(job_id: str, model: str, message_id: str):
    ...
    async for data in token_stream.subscribe_tokens(job_id):
        if data.get("text") == token_stream.DONE_SENTINEL:
            break
        # Works with both the faithful delta protocol and legacy workers.
        # (Tool-use blocks come with the #53 Anthropic passthrough; for now we
        # render the answer text only.)
        text = token_stream.event_content_text(data)
        if not text:
            continue
        token_count += 1
...
async def _collect_response(job_id: str, model: str) -> dict:
    ...
    async for data in token_stream.subscribe_tokens(job_id):
        if data.get("text") == token_stream.DONE_SENTINEL:
            ...
            break
        full_text += token_stream.event_content_text(data)
        token_count += 1

    output_tokens = (usage or {}).get("completion_tokens", token_count)
    return fmt.anthropic_response(full_text, model, output_tokens=output_tokens)
